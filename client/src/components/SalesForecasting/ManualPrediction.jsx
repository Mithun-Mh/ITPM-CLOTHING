...
const ManualPrediction = () => {
  ...
  const validateInputs = () => {
    ...
    if (!productName || typeof productName !== "string" || productName.trim() === "") {
      setError("Product Name must be a non-empty string.");
      return false;
    }

    // Validate sales data for each month
    for (let i = 0; i < salesData.length; i++) {
      const value = Number(salesData[i]);
      if (isNaN(value) || value <= 0) {
        setError(`Sales for ${months[i]} must be a positive number greater than zero.`);
        return false;
      }
      if (value > 30000) {
        setError(`Sales for ${months[i]} must not exceed 30,000 units.`);
        return false;
      }
    }
